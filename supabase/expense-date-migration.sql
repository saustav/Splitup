-- Run in Supabase SQL Editor if create_expense / update_expense lack p_expense_date.
-- Safe to re-run: drops old RPC signatures and recreates with expense_date support.

alter table public.expenses
  add column if not exists expense_date date not null default current_date;

drop function if exists public.create_expense(uuid, text, numeric, uuid, jsonb, text);
drop function if exists public.update_expense(uuid, text, numeric, jsonb, uuid, text);
...
begin
  ...
  perform public.log_activity_event(
    ...
    p_amount,
    v_currency,
    jsonb_build_object('category', v_category, 'expense_date', v_expense_date)
  );
